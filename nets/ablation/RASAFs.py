...
class RFAB(nn.Module):
  def __init__(self, c=16,SAFFBn=6):
    super(RFAB, self).__init__()

    self.fst_blk = SAFFB(c, SAFFBn)
    self.snd_blk = SAFFB(c, SAFFBn)
    self.trd_blk = SAFFB(c, SAFFBn)
...
class UpsamplingBlock(nn.Module):
  def __init__(self, c, t=2, upscale=2):
    ...
    for _ in range(self.t):
      blocks.append(RFAB(c))                    #rfab not saffb
    #   blocks.append(SAFFB(c,6))               #saffb not rfab
  ...
  def forward(self, fm):
    ...
    for i in range(0, self.t):
      output = self.RFABs[i](output)          #saffbor rfab
    #   output = output                 #not rfab not saffb
